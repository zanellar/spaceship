...
class Drone3DStopMotion:
    ...
    def add_slits(self, slits, color='g', alpha=0.5, negative=False, extended_width=2, extended_height=2):
        """
        Adds slits to the scene
        @ slits: list of dicts with keys "height", "width", "position"
        """ 
    ...
    def add_drone(self, orientations, positions, color_disk='b', alpha_disk=0.5, color_normal='r', alpha_normal=0.5):
        ...
        for i,normal,position in zip(range(n),orientations,positions):
            ...
            rgb_color_disk= mcolors.to_rgb(color_disk)
            darkened_color_disk = tuple([c * 0.5 for c in rgb_color_disk])
            # rgb_color_normal = mcolors.to_rgb(color_disk)
            # darkened_color_normal = tuple([c * 0.5 for c in rgb_color_normal])
 
            # Add a circle to the plot
            circle = Circle((0,0), .2, facecolor = color_disk, alpha = alpha_disk, edgecolor = darkened_color_disk) 
